...
class FourSplitDecodingStrategy(DecodingStrategy):
    ...
    def _get_analysis_arrays(self, fs: float, quarter: int):
        ...
        # Use the largest integer-cycle window length that fits in quarter.
        cycles_per_period = max(1, int(round(fs / self._f0)))
        n_cycles = quarter // cycles_per_period
        if n_cycles < 1:
            analysis_len = quarter
        else:
            analysis_len = n_cycles * cycles_per_period
        win_start = (quarter - analysis_len) // 2  # centre window in each quarter
    ...
    def _decode(self, samples: List[float]) -> SymbolRow:
        ...
        total_harmonics = self._settings.total_harmonics
        quarter = self._internal_clock // 4

        analysis_len, win_start, hann_win, valid_mask, proj_p, proj_d = self._get_analysis_arrays(fs, quarter)

        samples_arr = np.asarray(samples, dtype=np.float64)
        xp = samples_arr[win_start:win_start + analysis_len] * hann_win
        xd = samples_arr[2 * quarter + win_start:2 * quarter + win_start + analysis_len] * hann_win
